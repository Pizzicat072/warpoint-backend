...
async function fetchWeather() {
    ...
    try {
        ...
        return { temperature: 0, temperatureDisplay: '0', description: 'Нет данных', icon: '???', isError: true };
    } catch (err) {
        console.error('? Ошибка парсинга:', err.message);
        if (page) await page.close().catch(() => {});
        if (lastWeatherData) return lastWeatherData;
        return { temperature: 0, temperatureDisplay: '0', description: 'Ошибка', icon: '???', isError: true };
    }
}
