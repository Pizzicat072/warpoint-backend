...
app.post('/api/data', async (req, res) => { saveData(req.body); await broadcastEvent('full-update', req.body); res.json({ success: true }); });

app.post('/api/employees', async (req, res) => {
    const { employee, profile, login, password } = req.body;
    const data = loadData();
    if (data.employees.includes(employee)) return res.status(400).json({ error: 'Сотрудник уже существует' });
...
app.post('/api/admin/bonus', async (req, res) => {
    ...
    const data = loadData();
    data.employees.forEach(e => { if (data.profiles[e]) { data.profiles[e].coins += coins; data.profiles[e].rating += rating; } });
    saveData(data);
...
app.post('/api/admin/reset', async (req, res) => {
    const initialData = { employees: [], profiles: {}, passwords: {}, tasks: [], fines: [], schedule: {}, messages: { general: [] } };
