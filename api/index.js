...
const pool = new Pool({
    ...
    ssl: {
        rejectUnauthorized: false // Важно для Render.com и Vercel!
    },
    max: 20,
...
// ============================================
// STATIC & START (ИСПРАВЛЕНО ДЛЯ VERCEL)
// ============================================
...
// ============================================
// ЭКСПОРТ ДЛЯ VERCEL (ИСПРАВЛЕНО!)
// ============================================

// Экспортируем приложение Express (ОБЯЗАТЕЛЬНО для Vercel)
module.exports = app;

// Флаг, чтобы инициализировать БД только один раз
let initialized = false;

// Middleware для инициализации БД при первом запросе
app.use(async (req, res, next) => {
    if (!initialized) {
        console.log('🔄 Первый запрос - инициализация базы данных...');
        await initDatabase();
        await initAchievements();
        initialized = true;
        console.log('✅ База данных и достижения инициализированы');
    }
    next();
});