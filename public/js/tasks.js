...
function renderTasksTable() {
    ...
    for (const task of filtered) {
        ...
        if (task.is_group_task === 'operators') {
            executorDisplay = '📢 Все операторы';
            isGroupTask = true;
        } else if (task.is_group_task === 'admins') {
            executorDisplay = '⚙️ Все администраторы';
            isGroupTask = true;
        }
...
function openTaskModal(taskId = null) {
    ...
    if (taskId) {
        ...
        if (task && !task.is_archived && task.status !== 'completed') {
            ...
            if (task.is_group_task === 'operators') {
                executorField.value = '__GROUP_OPERATORS__';
            } else if (task.is_group_task === 'admins') {
                executorField.value = '__GROUP_ADMINS__';
            } else {
                executorField.value = task.executor;
            }
