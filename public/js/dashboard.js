...
function renderStylesShop() {
    ...
    for (const style of availableStyles) {
        ...
        let buttonHtml = '';
        if (isCurrent) {
            buttonHtml = `<button class="style-btn style-btn-current" disabled>✅ Активен</button>`;
        } else if (isBought) {
            buttonHtml = `<button class="style-btn style-btn-apply" onclick="window.applyBoughtStyle('${style.id}')">🎨 Применить</button>`;
        } else {
            const canBuy = userCoins >= style.price || isDirector;
            buttonHtml = `<button class="style-btn style-btn-buy" onclick="window.buyStyle('${style.id}', ${style.price})" ${!canBuy ? 'disabled style="opacity:0.5"' : ''}>
                            Купить за ${style.price} 🪙
                          </button>`;
        }
        ...
        html += `
            <div class="style-card ${isBought ? 'bought' : ''} ${isCurrent ? 'current' : ''}" 
                 data-style-id="${style.id}"
                 onmouseenter="window.previewStyle('${style.id}')" 
                 onmouseleave="window.cancelPreview()">
...
function initStylesShop() {
    renderStylesShop();
}

async function buyStyle(styleId, price) {
    if (!window.app) {
        showNotif('Ошибка: данные пользователя не загружены', 'error');
        return false;
    }
    
    if (window.app.currentUserRole === 'director') {
        showNotif('Директору все стили доступны бесплатно!', 'warning');
        return false;
    }
...
async function applyBoughtStyle(styleId) {
    ...
    if (window.app.userStyle === styleId) {
        showNotif('Этот стиль уже активен!', 'info');
        return false;
    }
    ...
    try {
        ...
        if (response && response.success) {
            ...
            if (styleId !== 'standart') {
                ...
                console.log(`🎨 Применён стиль дашборда: dashboard-style-${styleId}`);
            } else {
                document.body.classList.add('dashboard-style-standart');
                console.log('🎨 Применён стиль дашборда: standart');
            }
...
function loadHiddenBlocks() {
    ...
    if (saved) {
        try {
            hiddenBlocks = new Set(JSON.parse(saved));
        } catch(e) {}
    }
}
...
function updateTogglesState() {
    ...
    blockIds.forEach(blockId => {
        ...
        if (checkbox) {
            checkbox.checked = !hiddenBlocks.has(blockId);
        }
    });
}
...
function applyDashboardPreset(presetName) {
    const presets = {
        ...
        team: ['welcome-panel', 'activity-grid', 'key-stats', 'quote-card'],
    ...
    blockIds.forEach(blockId => {
        ...
        if (checkbox) {
            checkbox.addEventListener('change', (e) => {
                toggleBlockVisibility(blockId, e.target.checked);
            });
        }
    });
}
...
async function fetchWeather() {
    ...
    try {
        ...
        if (data && data.success) {
            ...
            if (tempEl) {
                if (data.tempDisplay) {
                    tempEl.textContent = data.tempDisplay + '°C';
                } else {
            ...
            if (feelsLikeEl) {
                if (data.feelsLikeDisplay) {
                    feelsLikeEl.innerHTML = 'ощущается как ' + data.feelsLikeDisplay + '°C';
                } else {
...
function loadHolidaysAndBirthdays() {
    ...
    for (const emp of employees) {
        ...
        if (profile && profile.birthday) {
            ...
            if (birthDate && !isNaN(birthDate.getTime())) {
                ...
                const isToday = (nextBirthday.getDate() === currentDate && nextBirthday.getMonth() === currentMonth);
    ...
    for (const holiday of holidays) {
        ...
        const isToday = (eventDate.getDate() === currentDate && eventDate.getMonth() === currentMonth);
    ...
    for (const event of upcomingEvents) {
        ...
        html += `
            <div style="display: flex; align-items: center; gap: 14px; padding: 10px 14px; background: rgba(0, 0, 0, 0.2); border-radius: 12px; border-left: 2px solid ${event.isToday ? '#fbbf24' : eventColor}; margin-bottom: 8px;">
                <div style="flex: 1;">
                    <div style="font-weight: 500; font-size: var(--font-sm);">${event.name}</div>
                    <div style="font-size: var(--font-xs); color: #64748b; margin-top: 2px;">${dateStr}</div>
                </div>
...
function updateNextShiftInfo() {
    ...
    const todayShift = schedule[today]?.[currentUser];
    const hasShiftToday = todayShift && todayShift.time;
    ...
    if (hasShiftToday) {
        if (currentTimeMinutes < SHIFT_START) {
            const minutesToStart = SHIFT_START - currentTimeMinutes;
            const timeStr = formatTimeMinutes(minutesToStart);
        ...
        else if (currentTimeMinutes >= SHIFT_START && currentTimeMinutes < SHIFT_END) {
            const totalDuration = SHIFT_END - SHIFT_START;
            const elapsed = currentTimeMinutes - SHIFT_START;
            const percent = Math.min(100, Math.max(0, Math.round((elapsed / totalDuration) * 100)));
            const minutesLeft = SHIFT_END - currentTimeMinutes;
            const timeLeft = formatTimeMinutes(minutesLeft);
    ...
    for (const [date, shifts] of Object.entries(schedule)) {
        if (date > today && shifts[currentUser] && shifts[currentUser].time) {
            if (!nearestDate || date < nearestDate) {
                nearestDate = date;
                nearestShift = shifts[currentUser];
            }
        }
    }
    
    if (nearestShift) {
        const shiftDate = new Date(nearestDate);
        const todayDate = new Date();
        todayDate.setHours(0, 0, 0, 0);
        const daysDiff = Math.ceil((shiftDate - todayDate) / (1000 * 60 * 60 * 24));
        
        let timeUntil = '';
        if (daysDiff === 0) timeUntil = 'сегодня';
        else if (daysDiff === 1) timeUntil = 'завтра';
        else timeUntil = `через ${daysDiff} дня`;
...
function quickChangeStatus(status) {
    if (window.app?.currentUser) {
        updateEmployeeStatus(window.app.currentUser, status).then(() => {
...
async function loadDailyBonusInfo() {
    try {
        ...
        if (response && response.success) {
            const streakDays = response.streak || 0;
            const hasClaimedToday = response.hasClaimedToday;
            ...
            if (claimBtn) {
                if (hasClaimedToday) {
                    ...
                    claimBtn.innerHTML = '✅ Получен';
                    claimBtn.style.background = '#64748b';
                } else {
...
async function claimDailyBonus() {
    ...
    try {
        ...
        if (response && response.claimed) {
            ...
            showNotif(`🎁 Вы получили ${response.bonus} WP! День ${response.streak}`, 'success');
            
            await loadDailyBonusInfo();
            await updateUserBalance();
            
            if (response.streak === response.maxStreak) {
                showNotif('👑 Вы достигли максимума стрика! Выдан статус "⭐ MVP"', 'success');
                if (typeof updateUserAvatarAndStatus === 'function') updateUserAvatarAndStatus();
                if (typeof renderEmployees === 'function') renderEmployees();
            }
        } else if (response && !response.claimed) {
            showNotif('❌ Бонус уже получен сегодня', 'warning');
            await loadDailyBonusInfo();
        } else {
            showNotif('❌ Ошибка при получении бонуса', 'error');
            if (claimBtn) {
                claimBtn.disabled = false;
                claimBtn.innerHTML = '🎁 Забрать';
            }
...
async function loadMyExchanges() {
    try {
        const response = await apiCall('/exchange/my');
        if (response && response.success) {
            window.myExchanges = response.requests || [];
        }
    } catch (err) {
        ...
        html += `
            <div class="exchange-notification">
                <div class="exchange-header">
                    ...
                    <span class="exchange-time">${new Date(req.created_at).toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
                <div class="exchange-body">
                    <div class="exchange-employee">👤 ${escapeHtml(req.from_employee)} хочет обменяться</div>
                    <div class="exchange-details">
                        <div class="exchange-detail">📌 Его смена: ${fromDate} — ${req.from_shift_time}</div>
                        <div class="exchange-detail">📌 Ваша смена: ${toDate} — ${req.to_shift_time}</div>
...
// ============================================
// ОТКРЫТИЕ МОДАЛКИ С ИСТОРИЕЙ ТРАНЗАКЦИЙ
// ============================================
        ...
        html += `
            <div class="my-exchange-request">
                <div class="my-exchange-header">
                    <span class="my-exchange-icon">📤</span>
                    <span class="my-exchange-title">Запрос отправлен</span>
                    <span class="my-exchange-status">⏳ ожидает</span>
                </div>
                <div class="my-exchange-body">
                    <div class="my-exchange-detail">
                        <span class="my-exchange-label">Кому:</span>
                        <span class="my-exchange-value">${escapeHtml(req.to_employee)}</span>
                    </div>
                    <div class="my-exchange-detail">
                        <span class="my-exchange-label">Моя смена:</span>
                        <span class="my-exchange-value">${fromDate} — ${req.from_shift_time}</span>
                    </div>
                    <div class="my-exchange-detail">
                        <span class="my-exchange-label">Его смена:</span>
                        <span class="my-exchange-value">${toDate} — ${req.to_shift_time}</span>
                    </div>
...
function initSettingsTabs() {
    ...
    function switchPanel(activePanel, activeBtn) {
        ...
        const btns = [btnStyles, btnPresets, btnBlocks];
        btns.forEach(btn => {
            if (btn) {
...
// ============================================
// ОСНОВНАЯ ФУНКЦИЯ INITDASHBOARD
// ============================================
...
function initDashboard() {
    ...
    if (currentUser && window.app.profiles[currentUser]) {
        ...
        console.log('🔄 Загружен стиль из профиля:', savedStyle);
    }
    ...
    if (savedPreset && ['default', 'compact', 'focus', 'team', 'minimal'].includes(savedPreset)) {
    ...
    console.log(`🔄 Загружен стиль дашборда: ${savedStyle}`);
