...
(function() {
    ...
    async function updateDashboardStats() {
        ...
        try {
            ...
            const tasksText = document.getElementById('tasksProgressText'); if (tasksText) tasksText.innerHTML = `${tasksDone} из ${tasksTotal}`;
            const tasksBar = document.getElementById('tasksProgressBar'); if (tasksBar) tasksBar.style.width = tasksTotal > 0 ? (tasksDone / tasksTotal * 100) + '%' : '0%';
            ...
            const finesCount = fines.filter(f => { const fineDate = new Date(f.date); const now = getTobolskNow(); return fineDate.getMonth() === now.getMonth() && fineDate.getFullYear() === now.getFullYear(); }).length;
            const statFinesCount = document.getElementById('statFinesCount'); if (statFinesCount) statFinesCount.textContent = finesCount;
            const fundAmount = await loadFundAmount();
            const fundAmountSpan = document.getElementById('statFundAmount'); if (fundAmountSpan) fundAmountSpan.textContent = (fundAmount || 0).toLocaleString() + ' ₽';
            const salaryFundEl = document.getElementById('salaryFundAmount'); if (salaryFundEl) salaryFundEl.textContent = (fundAmount || 0).toLocaleString() + ' ₽';
    ...
    function getCurrentBalance() { const currentUser = window.app?.currentUser; if (currentUser && window.app?.profiles?.[currentUser]) return window.app.profiles[currentUser].coins || 0; return 0; }
    ...
    // НАСТРОЙКИ ДАШБОРДА
    // ============================================
    function toggleDashboardSettings() { const content = document.getElementById('settingsContent'); const icon = document.getElementById('settingsToggleIcon'); if (!content) return; if (content.style.display === 'none' || content.style.display === '') { content.style.display = 'block'; if (icon) icon.style.transform = 'rotate(180deg)'; } else { content.style.display = 'none'; if (icon) icon.style.transform = 'rotate(0deg)'; } }
    function initSettingsTabs() { const btnStyles = document.getElementById('btnStyles'), btnPresets = document.getElementById('btnPresets'), btnBlocks = document.getElementById('btnBlocks'), panelStyles = document.getElementById('panelStyles'), panelPresets = document.getElementById('panelPresets'), panelBlocks = document.getElementById('panelBlocks'); if (!btnStyles || !panelStyles) return; function switchPanel(activePanel, activeBtn) { if (panelStyles) panelStyles.style.display = 'none'; if (panelPresets) panelPresets.style.display = 'none'; if (panelBlocks) panelBlocks.style.display = 'none'; if (activePanel) activePanel.style.display = 'block'; [btnStyles, btnPresets, btnBlocks].forEach(btn => { if (btn) { btn.style.background = 'transparent'; btn.style.color = '#94a3b8'; } }); if (activeBtn) { activeBtn.style.background = 'rgba(99,102,241,0.2)'; activeBtn.style.color = '#a78bfa'; } } btnStyles.onclick = () => switchPanel(panelStyles, btnStyles); btnPresets.onclick = () => switchPanel(panelPresets, btnPresets); btnBlocks.onclick = () => switchPanel(panelBlocks, btnBlocks); panelStyles.style.display = 'block'; panelPresets.style.display = 'none'; panelBlocks.style.display = 'none'; btnStyles.style.background = 'rgba(99,102,241,0.2)'; btnStyles.style.color = '#a78bfa'; }
    ...
    function updateNextShiftInfo() {
        ...
        const nextShiftEl = document.getElementById('nextShiftInfo'), shiftTimerEl = document.getElementById('shiftTimer'); if (!nextShiftEl || !shiftTimerEl) return;
        const schedule = window.app?.schedule || {}; const now = getTobolskNow(); const today = now.toISOString().split('T')[0];
        let nextShiftDate = null, nextShiftTime = null;
        const dates = Object.keys(schedule).sort();
        for (const date of dates) { if (date < today) continue; const daySchedule = schedule[date]; const myShift = daySchedule[currentUser]; if (myShift && myShift.time && myShift.status !== 'dayoff' && myShift.status !== 'sick') { nextShiftDate = date; nextShiftTime = myShift.time; break; } }
        if (nextShiftDate && nextShiftTime) {
            const shiftDate = new Date(nextShiftDate); const formattedDate = shiftDate.toLocaleDateString('ru-RU', { day: 'numeric', month: 'long' }); nextShiftEl.textContent = `${formattedDate} в ${nextShiftTime}`;
            const [hours, minutes] = nextShiftTime.split(':').map(Number); const shiftDateTime = new Date(nextShiftDate); shiftDateTime.setHours(hours, minutes, 0, 0);
            const updateTimer = () => { const now = getTobolskNow(); const diff = shiftDateTime - now; if (diff <= 0) { shiftTimerEl.textContent = 'Уже началась!'; return; } const days = Math.floor(diff / (1000 * 60 * 60 * 24)); const hrs = Math.floor((diff % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)); const mins = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60)); if (days > 0) shiftTimerEl.textContent = `${days} дн ${hrs} ч`; else shiftTimerEl.textContent = `${hrs} ч ${mins} мин`; };
            updateTimer(); if (shiftTimerInterval) clearInterval(shiftTimerInterval); shiftTimerInterval = setInterval(updateTimer, 60000);
        } else { nextShiftEl.textContent = 'Нет запланированных смен'; shiftTimerEl.textContent = '—'; }
    }
    ...
    // ЦИТАТА ДНЯ
    // ============================================
    function refreshPhilosophyQuote() { const quotes = [{ text: "Работа избавляет нас от трёх великих зол: скуки, порока и нужды.", author: "Вольтер" }, { text: "Успех — это способность двигаться от неудачи к неудаче, не теряя энтузиазма.", author: "Черчилль" }, { text: "Единственный способ сделать великую работу — любить то, что вы делаете.", author: "Стив Джобс" }, { text: "Секрет успеха — начать.", author: "Марк Твен" }, { text: "Лучший способ предсказать будущее — изобрести его.", author: "Алан Кей" }, { text: "Не бойтесь совершенства, вам его не достичь.", author: "Сальвадор Дали" }]; const quote = quotes[Math.floor(Math.random() * quotes.length)]; const quoteText = document.getElementById('philosophyQuoteText'); const quoteAuthor = document.getElementById('philosophyQuoteAuthor'); if (quoteText) quoteText.textContent = quote.text; if (quoteAuthor) quoteAuthor.textContent = `— ${quote.author}`; }
    ...
    function initDashboard() {
        ...
        const savedPreset = localStorage.getItem('dashboardPreset'); if (savedPreset && ['default', 'compact', 'focus', 'minimal'].includes(savedPreset)) applyDashboardPreset(savedPreset);
        ...
        dashboardInterval = setInterval(() => { updateDashboardStats(); loadActivity(); updateNextShiftInfo(); loadPendingExchanges(); }, 60000);
        if (bonusCheckInterval) clearInterval(bonusCheckInterval);
        bonusCheckInterval = setInterval(() => { loadDailyBonusInfo(); loadPendingExchanges(); }, 300000);
        if (window.channel) { window.channel.bind('schedule-updated', (data) => { if (typeof loadScheduleData === 'function') loadScheduleData(); updateNextShiftInfo(); updateDashboardStats(); }); }
