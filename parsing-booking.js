// parsing-booking.js — АДАПТИРОВАН ПОД ПРОЕКТ
...
async function getBrowser() {
    ...
    if (sharedBrowser && sharedBrowser.isConnected()) {
        try {
            ...
            console.log('⚠️ Браузер отвалился, пересоздаём...');
            sharedBrowser = null;
        }
    }
    ...
    browserInitPromise = (async () => {
        try {
            ...
            // Используем @sparticuz/chromium для serverless
            ...
            console.log('🔄 Пробуем обычный puppeteer...');
            sharedBrowser = await puppeteer.launch({
...
class BookingParser {
    ...
    async parseAvailability() {
        if (isParsing) {
            console.log('⚠️ Парсинг уже выполняется');
